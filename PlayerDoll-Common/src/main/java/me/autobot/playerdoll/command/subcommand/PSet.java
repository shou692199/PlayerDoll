...
public class PSet extends SubCommand implements DollCommandExecutor {
    ...
    private void openGUI() {
        if (sender instanceof Player player) {
            player.openInventory(DollGUIHolder.DOLL_GUI_HOLDERS.get(target.getUniqueId()).getPSetMenu(Bukkit.getOfflinePlayer(profile.getId())).getInventory());
        }
    }
...
//        }
        ...
        if (!outputHasPerm(sender, DollConfig.getOnlineDollConfig(target.getUniqueId()), FlagConfig.PersonalFlagType.PSET)) {
            return 0;
        }
